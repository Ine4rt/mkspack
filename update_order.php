...
if ($field === 'completed') {
    ...
    $stmt = $conn->prepare("UPDATE orders SET scan = ? WHERE id = ?");
    $stmt->bind_param("ii", $value, $orderId);
...
elseif ($field === 'reprise') {  
    ...
    if ($value == 1) {
    $newRepriseDate = date('Y-m-d');  // Toujours mettre une date quand reprise = 1
} else {
    $newRepriseDate = NULL;  // Si reprise = 0 on efface
}
...
elseif ($field === 'jacket_missing' || $field === 'pants_missing' || $field === 'kit_missing' || $field === 'under_shirt_missing' || $field === 'short_missing' || $field === 'bas_missing' || $field === 'jersey_missing' || $field === 'polo_missing' || $field === 'option_bas_missing' || $field === 'option_kway_missing') {  
