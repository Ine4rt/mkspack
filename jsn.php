...
let initPrix = (init && init.trim() !== '') ? '30€' : '';
...
let titrePack = 'Pack JSN'; // par défaut

if (role.trim() === 'Joueur-JSN') {
    titrePack = 'Pack JSN';
} else if (role.trim() === 'Entraineur-JSN') {
    titrePack = 'Pack JSN Entraîneur';
}


if (role.trim() === "Ellas-Keeper") {
    var titles = {
        veste: "Veste ",
        pantalon: "Pantalon",
        kit: "Kit Keeper",
    ...
    var titles = {
        veste: "Veste Entraineur",
        pantalon: "Pantalon Entraineur",
    };
    var references = {
        veste: "100227101",
        pantalon: "100522101",
...
<div class="grid-container" style="grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); max-width: 400px; margin: 0 auto;">
    ...
    <div class="item-box" style="border: 2px solid #c00; background-color: #ffe6e6;">
        ...
        <div class="item-size" style="color: #900; font-weight: bold;">30€</div>
    </div>
</div>
` : `
...
<body>
    ...
    <div class="club-container">
        <img src="jsn.png" alt="JSN" class="club-logo">
        <span style="font-size: 40px; font-weight: bold;">JSN</span>
...
<div id="filterContainer">
    ...
    <button class="filter-btn" onclick="filterByRole('Joueur-JSN', this)">Joueur</button>
	<button class="filter-btn" onclick="filterByRole('Entraineur-JSN', this)">Entraineur</button>
</div>
...
<div style="text-align: right; margin-bottom: 10px;" id="checkboxFilters">
    ...
    <table>
        ...
        <tbody>
            ...
            $sql = "SELECT * FROM orders WHERE club = 'JSN'";
...
echo "<td>" . $date->format('d/m') . "</td>";
                ...
                echo "'>";
                if ($row['role'] == 'Entraineur-JSN') {
                    echo "👶🏻"; // Icône ballon de foot
                } elseif ($row['role'] == 'Joueur-JSN') {
...
function exportToExcel() {
    window.location.href = 'export_excel-jsn.php';
}
...
function toggleFilter() {
    ...
    if (currentFilter === "all") {
        currentFilter = "Joueur-JSN";
        button.textContent = "Afficher: Joueur-JSN";
    } else if (currentFilter === "Joueur-JSN") {
        currentFilter = "Entraineur-JSN";
        button.textContent = "Afficher: Entraineur-JSN";
    } else {
        currentFilter = "all";
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (currentFilter === "all" || role === "👶🏻" && currentFilter === "Entraineur-JSN" || role === "⚽️" && currentFilter === "Joueur-JSN") {
                row.style.display = "";
            } else {
...
function filterByRole(role, button) {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (role === "all" || (role === "Entraineur-JSN" && roleText === "👶🏻") || (role === "Joueur-JSN" && roleText === "⚽️")) {
                row.style.display = "";
            } else {
                row.style.display = "none";
            }
...
function applyFilters() {
    ...
    rows.forEach(row => {
        ...
        const cotiInput = row.querySelector("td:nth-child(12) input");
        const coti = cotiInput && cotiInput.value.trim() !== "";

        const factureInput = row.querySelector("td:nth-child(13) input");
        const facture = factureInput && factureInput.value.trim() !== "";

        const reprisInput = row.querySelector("td:nth-child(14) input");
        const repris = reprisInput && reprisInput.value.trim() !== "";

        const termineBtn = row.querySelector("td:nth-child(21) button");
        const termine = termineBtn && getComputedStyle(termineBtn).backgroundColor === "rgb(0, 128, 0)";
    ...
    if (
        showManquant || showCoti || showFacture || showRepris 
    ) {
        document.getElementById("filter_tout").checked = false;
    }
