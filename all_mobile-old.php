...
.edit:hover {
    ...
    input[type="date"] {
        ...
        /* Ligne grise pour email interne */
        tr.grey-row td,
        tr.grey-row {
...
$sql = "SELECT * FROM orders WHERE club = '" . $conn->real_escape_string($club) . "'";
			...
			$scanClass = ($row['scan'] == 1) ? 'scan-yellow' : '';
                ...
                echo "<tr data-order-id='" . htmlspecialchars($row['id']) . "' data-club='" . htmlspecialchars($row['club']) . "' class='" . $completedClass . ' ' . $greyClass . "'>";
...
if (showManquant) {
    ...
    rows.forEach(row => {
        ...
        if (isMobile) {
            // MODE MOBILE : chercher dans les .size-item
            ...
            sizeItems.forEach(item => {
                ...
                if (label) {
                    ...
                    summary[label][value] = (summary[label][value] || 0) + 1;
                }
            });
        } else {
            // MODE DESKTOP : logique originale
            const cells = row.querySelectorAll("td");
