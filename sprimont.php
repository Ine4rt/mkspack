...
let titrePack = 'Pack RFCB Sprimont'; // par défaut

if (role.trim() === 'Ellas-Keeper') {
    titrePack = 'Pack Ellas Keeper';
} else if (role.trim() === 'Entraineur') {
    titrePack = 'Pack Sprimont Entraîneur';
}
...
if (role.trim() === "Ellas-Keeper") {
    ...
    var titles = {
        ...
        short: "Short Entraineur",
		jersey: "Maillot Entraineur",
    };
    var references = {
        veste: "100226901",
        pantalon: "100522101",
...
<div id="filterContainer">
    ...
    <button class="filter-btn" onclick="filterByRole('U10-U21', this)">Joueur</button>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>2 Maillot </br> 100225340</th>
               
				<th>2 Bas </br> 100369711</th>
				 <th>Short </br> 100530641</th>
                
                <th>Polo </br> 100227643</th>
                <th>Initiales</th>
...
function exportToExcel() {
    window.location.href = 'export_excel-sprimont.php';
}
...
function toggleFilter() {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (currentFilter === "all" || role === "👶🏻" && currentFilter === "Entraineur" || role === "⚽️" && currentFilter === "U10-U21") {
                row.style.display = "";
            } else {
...
function filterByRole(role, button) {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (role === "all" || (role === "Entraineur" && roleText === "👶🏻") || (role === "U10-U21" && roleText === "⚽️")) {
                row.style.display = "";
            } else {
                row.style.display = "none";
            }
...
function applyFilters() {
    ...
    rows.forEach(row => {
        ...
        const cotiInput = row.querySelector("td:nth-child(16) input");
        const coti = cotiInput && cotiInput.value.trim() !== "";

        const factureInput = row.querySelector("td:nth-child(17) input");
        const facture = factureInput && factureInput.value.trim() !== "";

        const reprisInput = row.querySelector("td:nth-child(18) input");
