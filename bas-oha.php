...
if (kway !== 'NON') {
    prixTotal += 19;  // Ajouter 19€ pour le K-way
}

if (option_bas !== 'NON') {
    prixTotal += 6;  // Ajouter 6€ pour les bas
}
...
let titrePack = 'Pack WBO'; // par défaut

if (role.trim() === 'Keeper-WBO') {
    titrePack = 'Pack WBO Keeper';
} else if (role.trim() === 'Joueur-WBO') {
    titrePack = 'Pack WBO';
}
...
<div class="grid-container" style="grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); margin: 0 auto;">
                ...
                <div class="item-size">${initialesHTML}</div>
    ...
    <div class="item-box" style="border: 2px solid #c00; background-color: #ffe6e6;">
        ...
        <div class="item-size" style="color: #900; font-weight: bold;">${prixTotal}€</div>
    </div>
</div>
...
<div id="filterContainer">
    ...
    <button class="filter-btn" onclick="filterByRole('Joueur-WBO', this)">Joueur</button>
    <button class="filter-btn" onclick="filterByRole('Keeper-WBO', this)">Keeper</button>
</div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Veste </br> 100226941</th>
                <th>Pantalon </br> 100522102</th>
                <th>Bas </br> 100369706</th>
                <th>Sous-pull </br> 100307814</th>
                <th>Maillot </br> 100225344</th>
				<th>Short </br> 100530641</th>
				<th>Opt Kway </br> 100529960</th>
				<th>Opt Bas </br> 100369706</th>
                <th>Initiales</th>
...
function exportToExcel() {
    window.location.href = 'export_excel-wbo.php';
}
...
function filterByRole(role, button) {
    ...
    rows.forEach(row => {
        ...
        if (roleCell) {
            ...
            if (
    role === "all" ||
    (role === "Joueur-WBO" && roleText === "⚽️") ||
    (role === "Keeper-WBO" && roleText === "🧤") ||
...
function applyFilters() {
    ...
    rows.forEach(row => {
        ...
        const cotiInput = row.querySelector("td:nth-child(18) input");
        const coti = cotiInput && cotiInput.value.trim() !== "";

        const factureInput = row.querySelector("td:nth-child(19) input");
        const facture = factureInput && factureInput.value.trim() !== "";

        const reprisInput = row.querySelector("td:nth-child(20) input");
