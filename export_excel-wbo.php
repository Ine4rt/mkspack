...
header("Content-Disposition: attachment; filename=commandes_wbo.xls");
...
echo "Pack\tDate\tNom\tPrénom\tCatégorie\tTéléphone\tEmail\tRôle\tVeste\tPantalon\tBas\tSous-Pull\tMaillot\tShort\tOpt. Kway\tOpt. Bas\tInitiales\tCoti Payée\tFacturé\tTerminé\n";
...
while ($row = $result->fetch_assoc()) {
	...
	echo htmlspecialchars($row['under_shirt_size']) . "\t";
	    echo htmlspecialchars($row['jersey_size']) . "\t";
			    echo htmlspecialchars($row['short_size']) . "\t";
					    echo htmlspecialchars($row['option_kway']) . "\t";
							    echo htmlspecialchars($row['option_bas']) . "\t";
